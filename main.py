...
class LocationsInfo:
    ...
    def read_cycle_parks(cls, cycleparks_url: str):
      ...
      if not os.path.exists(cache_file_name):
        ...
        req = urllib.request.Request(cycleparks_url, headers={'User-Agent': 'Mozilla/5.0'})
        with urllib.request.urlopen(req) as url:
          data = json.load(url)
          cls.location_data = data['features']
          logger.info('Loaded cycle parks data from %s; Saving to %s', cycleparks_url, cache_file_name)
      ...
      coords = np.radians([list(reversed(entry["geometry"]['coordinates'])) for entry in cls.location_data])
...
async def limit_locations(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    current_limit = context.user_data.get("locations_limit", LocationsInfo.DEFAULT_LOCATIONS_LIMIT)
    if not args:
...
async def show_nearest_cycleparks(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    locations_limit = context.user_data.get("locations_limit", LocationsInfo.DEFAULT_LOCATIONS_LIMIT)
    nearest_parkings, distances = LocationsInfo.get_nearest_cycleparks(lat, lon, k=locations_limit)

    logger.info('Retrieved %d nearest cycle parks within distances %r', len(nearest_parkings), distances)
    for i, (distance, parking_info) in enumerate(zip(distances, nearest_parkings)):
        ...
        image_url = props.get('PHOTO1_URL')
...
def main() -> None:
    ...
    application.add_handler(MessageHandler(filters.LOCATION, show_nearest_cycleparks))
